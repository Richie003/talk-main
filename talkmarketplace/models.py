...
def secondary_images_upload_path(instance, filename):
    return f"products/imgs/{instance.product.service_provider.talk_id}/secondary/{slugify(instance.product.name)}-{filename}"
...
class Product(ModelUtilsMixin, PolymorphicModel):
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2, validators=[
                                MinValueValidator(0)], null=False)
    address = models.CharField(max_length=255, null=False)
    negotiable = models.BooleanField(default=False)
    primary_image = models.ImageField(
        upload_to=primary_image_upload_path, null=True, blank=True)
...
class MarketPlaceProduct(Product):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
                             null=False, related_name='marketplace_products')
    ...
    def get_average_rating(self):
        if self.marketplace_reviews.exists():
            total_rating = sum(
                review.rating for review in self.marketplace_reviews.all())
...
class MarketPlaceProductImage(ModelUtilsMixin):
    product = models.ForeignKey(
        MarketPlaceProduct, on_delete=models.CASCADE, related_name='marketplace_images')
    image = models.ImageField(
        upload_to=secondary_images_upload_path, null=True, blank=True)
...
class MarketPlaceProductVideo(ModelUtilsMixin):
    product = models.ForeignKey(
        MarketPlaceProduct, on_delete=models.CASCADE, related_name='marketplace_videos')
...
class MarketPlaceProductReview(ModelUtilsMixin):
    product = models.ForeignKey(
        MarketPlaceProduct, on_delete=models.CASCADE, related_name='marketplace_reviews')
...
class TakaProduct(Product):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null=False, related_name='taka_products')
    category = models.ForeignKey(
        'Category', on_delete=models.CASCADE, null=False)
    ...
    def get_average_rating(self):
        if self.taka_reviews.exists():
            total_rating = sum(
                review.rating for review in self.taka_reviews.all())
            return total_rating / self.get_review_count()
        return 0
...
class TakaProductImage(ModelUtilsMixin):
    product = models.ForeignKey(
        TakaProduct, on_delete=models.CASCADE, related_name='taka_images')
    image = models.ImageField(
        upload_to=secondary_images_upload_path, null=True, blank=True)
...
class TakaProductVideo(ModelUtilsMixin):
    product = models.ForeignKey(
        TakaProduct, on_delete=models.CASCADE, related_name='taka_videos')
    video_path = models.FileField(
        upload_to=secondary_images_upload_path, null=True, blank=True)
    video_url = models.URLField(null=True, blank=True)
...
class TakaReview(ModelUtilsMixin):
    product = models.ForeignKey(
        TakaProduct, on_delete=models.CASCADE, related_name='taka_reviews')
    user = models.ForeignKey(settings.AUTH_USER_MODEL,
                             on_delete=models.CASCADE)
    rating = models.IntegerField(validators=[MinValueValidator(1)])
    comment = models.TextField(null=True, blank=True)
