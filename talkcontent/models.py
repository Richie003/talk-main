...
class Event(ModelUtilsMixin, CommonFields):
    ...
    def event_profile(self):
        return {
            ...
            "event_image": self.event_image.url if self.event_image else None,
            "event_date": self.event_date.strftime("%Y-%m-%d %H:%M:%S"),
            "event_fees": str(self.event_fees),
            "created_by": f"{self.user.first_name} {self.user.last_name}" if self.user else "Unknown",
            "created": self.created.strftime("%Y-%m-%d %H:%M:%S"),
            "updated": self.updated.strftime("%Y-%m-%d %H:%M:%S"),
        }
...
class SavedEventItem(ModelUtilsMixin):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    event = models.ManyToManyField(Event, related_name='saved_items')
    ...
    def is_item_saved(self, event):
        return self.event.filter(id=event.id).exists()
    
    def get_user_saved_items(self):
        return self.event.filter(saved_items__user=self.user)

    def get_saved_item_count(self):
        return self.event.count()
    
    def get_saved_item_details(self):
        return [item.event_profile() for item in self.event.all()]
    
    def get_saved_item_by_id(self, event_id):
        return self.event.filter(id=event_id).first().event_profile() if self.event.filter(id=event_id).exists() else None
